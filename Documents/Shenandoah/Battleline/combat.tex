...
\begin{enumerate}
  ...
  \item The Non-Phasing Player repeats stesp \#3 and \#4.
...
\begin{enumerate}
  \item The Attacker examines the ATTACKER'S COMBAT TABLES, crossgridding the number of Combat Factors he is attacking with his present Formation (NOTE: If some of the attacker's units are in Column Formation, all of the units are assumed to be in Column Formation for purposes of using this Table). The number found is the attacker's Basic Combat Table Number. For instance, if the attacker has 10 Combat Factors in Battle Formation, his Basic Combat Table is "6".

  \item The attacker then consults the chart of Table Modifiers for changes to his Basic Combat Table Number. For instance, if the attacker's Basic Combat Table is "6", and he is attacking an enemy from three different hexes and the defender is in Battle Formation on Slope terrain, the modifier is found by crossgridding to be "4". Six minus four equals two, so that the Modified Combat Table Number is "2". A die roll is required for all attacks from two or more hexes. If the die roll were 2 (two) then this would be added to the Modified Table Number (i.e. Table 6 - 4 + 2 = Table \#4).
