...
\newcommand{\mediumplayimage}[2]{%
  \begin{figure}[H]
    \centering
    \includegraphics[width=0.65\linewidth,height=0.38\textheight,keepaspectratio]{#1}
    \caption{#2}
  \end{figure}%
}
\newcommand{\anchoredplayimage}[2]{%
  \begin{figure}[H]
    \centering
    \includegraphics[width=0.9\linewidth,height=0.82\textheight,keepaspectratio]{#1}
    \caption{#2}
...
\newcommand{\americancommanddiagram}{%
  \begingroup
  ...
  \begin{figure}[H]
    ...
    \begin{tikzpicture}[x=1cm,y=1cm,font=\scriptsize,>=latex,
      commandarrow/.style={->,draw=black,line width=0.45pt},
      commandline/.style={draw=black,line width=0.45pt}]
      \node[inner sep=0] (washingtoncommand) at (-4.9,0) {\countergraphic{american-leader-washington}{1.15cm}};
      \node[anchor=west,font=\bfseries] at (-4.15,-0.13) {4 AP};
      \node[align=left,anchor=west,text width=7.1cm] at (-2.65,0.12) {The American player spends 4 activation points and uses Washington's 6 command span. 13 leaders have been activated.};

      \node[inner sep=0] (sullivancommand) at (-2.9,-2.0) {\countergraphic{american-leader-sullivan}{1.15cm}};
      \node[anchor=west] at (-2.15,-2.0) {Washington activates Sullivan for 3 CS};
      \draw[commandarrow] (washingtoncommand.south) -- (-5.2,-0.85) -- (-5.2,-2.0) -- (sullivancommand.west);

      \node[inner sep=0] (hitchcockcommand) at (-2.9,-4.45) {\countergraphic{american-leader-hitchcock}{1.15cm}};
      \node[inner sep=0] (sargentcommand) at (-1.15,-4.45) {\countergraphic{american-leader-sargent}{1.15cm}};
      \node[inner sep=0] (mcdougallcommand) at (0.60,-4.45) {\countergraphic{american-leader-mcdougall}{1.15cm}};
      \node[inner sep=0] (mercercommand) at (2.35,-4.45) {\countergraphic{american-leader-mercer}{1.15cm}};
      \node[anchor=west] at (-2.9,-3.12) {Sullivan activates 4 leaders for 4 CS};
      \draw[commandarrow] (sullivancommand.south) -- (-2.9,-3.55);
      \draw[commandline] (-2.9,-3.55) -- (2.35,-3.55);
      \draw[commandarrow] (-2.9,-3.55) -- (hitchcockcommand.north);
      \draw[commandarrow] (-1.15,-3.55) -- (sargentcommand.north);
      \draw[commandarrow] (0.60,-3.55) -- (mcdougallcommand.north);
      \draw[commandarrow] (2.35,-3.55) -- (mercercommand.north);

      \node[inner sep=0] (greenecommand) at (-2.9,-6.5) {\countergraphic{american-leader-greene}{1.15cm}};
      \node[anchor=west] at (-2.15,-6.5) {Washington activates Greene for 3 CS};
      \draw[commandarrow] (-5.2,-2.0) -- (-5.2,-6.5) -- (greenecommand.west);
      \node[rotate=90,font=\footnotesize] at (-5.55,-4.25) {Activates};

      \node[inner sep=0] (cadwaladercommand) at (-2.9,-8.85) {\countergraphic{american-leader-cadwalader}{1.15cm}};
      \node[inner sep=0] (mifflincommand) at (-1.15,-8.85) {\countergraphic{american-leader-mifflin}{1.15cm}};
      \node[inner sep=0] (stirlingcommand) at (0.60,-8.85) {\countergraphic{american-leader-lord-stirling}{1.15cm}};
      \node[inner sep=0] (fermoycommand) at (2.35,-8.85) {\countergraphic{american-leader-fermoy}{1.15cm}};
      \node[anchor=west] at (-2.9,-7.52) {Greene activates 4 leaders for 5 CS};
      \draw[commandarrow] (greenecommand.south) -- (-2.9,-7.95);
      \draw[commandline] (-2.9,-7.95) -- (2.35,-7.95);
      \draw[commandarrow] (-2.9,-7.95) -- (cadwaladercommand.north);
      \draw[commandarrow] (-1.15,-7.95) -- (mifflincommand.north);
      \draw[commandarrow] (0.60,-7.95) -- (stirlingcommand.north);
      \draw[commandarrow] (2.35,-7.95) -- (fermoycommand.north);

      \node[inner sep=0] (dickinsoncommand) at (-2.9,-11.3) {\countergraphic{american-leader-dickinson}{1.15cm}};
      \node[inner sep=0] (detachmentcommand) at (-1.15,-11.3) {\countergraphic{american-detachment-4}{1.15cm}};
      \node[anchor=west] at (-2.9,-9.92) {Cadwalader activates 2 leaders for 2 CS};
      \draw[commandarrow] (cadwaladercommand.south) -- (-2.9,-10.35);
      \draw[commandline] (-2.9,-10.35) -- (-1.15,-10.35);
      \draw[commandarrow] (-2.9,-10.35) -- (dickinsoncommand.north);
      \draw[commandarrow] (-1.15,-10.35) -- (detachmentcommand.north);

      \node[align=left,anchor=west,font=\footnotesize] at (0.2,-11.65) {AP = Activation Point Cost\\CS = Command Span Used};
    \end{tikzpicture}
    }
    \caption{Example of Washington's maximum command.}
  \end{figure}%
  \endgroup
}
\newcommand{\washingtonsetupdiagram}{%
  \begin{figure}[htbp]
    \centering
    \begin{tikzpicture}[font=\small]
      \fill[black!12,rounded corners=4pt] (-5.55,0.95) rectangle (5.55,-5.35);
      \node[font=\large] at (0,0.42) {Within one hex of 1335};

      \node[inner sep=0] at (-3.5,-1.15) {\countergraphic{american-leader-lord-stirling}{1.35cm}};
      \node[inner sep=0] at (0,-1.15) {\countergraphic{american-leader-st-clair}{1.35cm}};
      \node[inner sep=0] at (3.5,-1.15) {\countergraphic{american-leader-fermoy}{1.35cm}};
      \node at (-3.5,-2.15) {\textbf{Troops:} 570};
      \node at (0,-2.15) {\textbf{Troops:} 420};
      \node at (3.5,-2.15) {\textbf{Troops:} 620};

      \node[inner sep=0] at (-3.5,-3.45) {\countergraphic{american-leader-glover}{1.35cm}};
      \node[inner sep=0] at (0,-3.45) {\countergraphic{american-leader-sargent}{1.35cm}};
      \node[inner sep=0] at (3.5,-3.45) {\countergraphic{american-leader-washington}{1.35cm}};
      \node at (-3.5,-4.45) {\textbf{Troops:} 850};
      \node at (0,-4.45) {\textbf{Troops:} 710};
      \node at (3.5,-4.45) {\textbf{Troops:} 140};
    \end{tikzpicture}
    \caption{Washington's initial setup.}
  \end{figure}%
}

\title{Washington's Crossing\\Campaign Game Example of Play}
\author{Daniel Berger (current document, \today)\\Roger Miller (original example, 2012)\\[1em]\footnotesize Originally collated from the Revolution Games website,\\\footnotesize and modified into the document that you see before you.}
\date{}
...
This playbook presents an example of the Campaign Game of \textit{Washington's Crossing}, the first game in the Campaigns of the American Revolution series. It is based on Roger Miller's original example of play, reworked for this edition with a new layout, recreated counter artwork, and clearer reference material. It introduces the game's central concepts through an actual playthrough.

\textit{Washington's Crossing} has two scenarios: the quick-start campaign, for those who wish to skip the initial Delaware River crossings by the Americans, and the Campaign Game, which is introduced here. The Campaign Game begins with General Washington preparing to cross the Delaware River to assault the Hessian garrison occupying Trenton, New Jersey. Before beginning, here is a brief overview of some game concepts.
...
Note that in practice you may find it easier to use a spreadsheet and pencil instead of the leader display. There are player aids both on the Revolution Games website as well as Boardgamegeek that you can use.
...
\begin{playchart}[0.72\linewidth]
  ...
  \begin{tabular}{|>{\raggedright\arraybackslash}m{0.27\linewidth}|*{3}{>{\centering\arraybackslash}m{0.16\linewidth}|}}
    \hline
    \rowcolor{black!10}
    \textbf{\shortstack[l]{\strut Modified\\[-0.15em]Die Roll *\strut}} & \textbf{2--4} & \textbf{5--6} & \textbf{7--8} \\
    \hline
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}[font=\large]
    \node[inner sep=0] (durhamboats) at (0,0) {\countergraphic{durham-boats}{1.35cm}};
    \node[anchor=west,font=\bfseries] at (1.05,0) {Hex 1330 or 1631};
  \end{tikzpicture}
  \caption{Durham Boats setup marker.}
\end{figure}
...
\begin{playchart}[0.72\linewidth]
  ...
  \par\smallskip
  \begin{minipage}{0.955\linewidth}\footnotesize\raggedright If night, subtract one from the die roll.\end{minipage}\par\smallskip
  \begin{tabular}{|>{\raggedright\arraybackslash}p{0.19\linewidth}|>{\centering\arraybackslash}p{0.69\linewidth}|}
    \hline
    \rowcolor{black!10}\textbf{Die Roll} & \textbf{Result} \\
    \hline
...
\begin{playchart}[0.75\linewidth]
  ...
  \par\smallskip
  \begin{minipage}{0.968\linewidth}\footnotesize\raggedright On night turns, the defending leader receives one additional fatigue on all results.\end{minipage}\par\smallskip
...
\mediumplayimage{WC_Turn2_files/Stephen_Crossing_haxh.JPG}{Stephen's crossing.}
...
\begin{playchart}[0.72\linewidth]
  ...
  {\renewcommand{\arraystretch}{1.30}
  \begin{tabular}{|>{\raggedright\arraybackslash}m{0.38\linewidth}|>{\centering\arraybackslash}m{0.25\linewidth}|>{\centering\arraybackslash}m{0.25\linewidth}|}
    \hline
    \rowcolor{black!10}\textbf{\shortstack[l]{\strut Modified\\[-0.15em]Die Roll *\strut}} & \textbf{CRT Modifier} & \textbf{Attacker \%} \\
    \hline
    ...
    10 & +3 & $-5$ \\ \hline
    11 & +3 & $-10$ \\ \hline
    \rowcolor{yellow!20}12 & +4 & $-10$ \\ \hline
    13 & +5 & $-10$ \\ \hline
...
\mediumplayimage{WC_Turn3_files/Rall_attack_SpxH.JPG}{Attack on Rall at Trenton.}
...
\mediumplayimage{WC_turn4_files/North_Turn4_HlS6.JPG}{Situation after Turn 4: British northern positions.}
\anchoredplayimage{WC_turn4_files/Central_Turn4_HlS6.JPG}{Situation after Turn 4: American central position.}
\mediumplayimage{WC_turn4_files/southern_turn4_HlS6.JPG}{Situation after Turn 4: Southern Front.}
...
\vfill
\begin{center}
\small Original text and images: \copyright\ 2012 Revolution Games, plus custom images from Daniel J. Berger. This document is a local collation of the downloaded example-of-play pages, excluding website navigation and decorative elements.
\end{center}
