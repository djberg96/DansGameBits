...
\begin{center}
  \includegraphics[width=0.54\linewidth]{playbook-turn-1-map.pdf}
\end{center}
...
\textbf{Movement.} From the Serbia Hide-away, move three Chetnik groups to
Pančevo (callout 2) and two to the Chetnik Mountain triangle. From the
Montenegro Hide-away, move both groups to Cetinje (callout 3). All movement is
within the units' starting Zones.

\textbf{Combat.} The Axis boxes of Pančevo and Cetinje are empty, so there is
no corresponding enemy stack and no combat \rulesref{8.11}.

\phase{C. Victory Point Stage}

Pančevo is worth 2 VP and Cetinje 1 VP. The Yugoslav Player gains
...
German 714 moves from Kragujevac to the Axis box at Pančevo (callout 4).
...
\textbf{Recruitment.} The two Chetnik groups at Cetinje occupy an Objective
with a Recruitment Value of one-half. The Yugoslav Player rolls \die{4}:

\[
  4 \times \tfrac{1}{2} = 2\text{ groups.}
\]

The result does not exceed the two occupying Yugoslav SP, so two new Chetnik
groups are placed at Cetinje \rulesref{7.44A}. Cetinje now contains four.
