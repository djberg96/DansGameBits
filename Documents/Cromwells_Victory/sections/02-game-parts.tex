...
\tikzset{
  ...
  piece lead/.style={draw=cvink!75,line width=.3pt}
}
...
\begin{tikzpicture}[x=1in,y=1in]
  ...
  \node[inner sep=0] at (2.66,.57)
    {\includegraphics[width=.55in]{images/rulebook-counters/urry-back.pdf}};

  \node[piece label,anchor=west] (uname) at (.02,1.01) {Name};
  \draw[piece lead] (uname.east) -- (1.16,.82);
  \node[piece label,anchor=west] (utype) at (.02,.70) {Type};
  \draw[piece lead] (utype.east) -- (1.03,.59);
  \node[piece label,anchor=east] (usetup) at (2.10,1.01) {Set-up Hex};
  \draw[piece lead] (usetup.west) -- (1.34,.60);
  \node[piece label,anchor=south west] (ucombat) at (.02,.02) {Combat\\Strength};
  \draw[piece lead] (ucombat.north east) -- (1.08,.31);
  \node[piece label,anchor=south] (umorale) at (1.18,.02) {Morale\\Rating};
  \draw[piece lead] (umorale.north) -- (1.16,.31);
  \node[piece label,anchor=south west] (umove) at (1.70,.02) {Movement\\Allowance};
  \draw[piece lead] (umove.north west) -- (1.25,.31);

  \node[piece label,font=\sffamily\fontsize{5.4}{5.8}\selectfont] at (1.16,1.08) {Front (ordered)};
...
\begin{tikzpicture}[x=1in,y=1in]
  ...
  \draw[piece lead] (lname.east) -- (1.16,.82);
  \node[piece label,anchor=west] (ltype) at (.02,.68) {Leader Type\\Symbol};
  \draw[piece lead] (ltype.east) -- (1.03,.58);
  \node[piece label,anchor=east] (lsetup) at (2.10,1.01) {Set-up Hex};
  \draw[piece lead] (lsetup.west) -- (1.34,.60);
  \node[piece label,anchor=south west] (lcommand) at (.02,.02) {Command\\Rating};
  \draw[piece lead] (lcommand.north east) -- (1.11,.31);
  \node[piece label,anchor=south west] (lmove) at (1.70,.02) {Movement\\Allowance};
  \draw[piece lead] (lmove.north west) -- (1.22,.31);

  \node[piece label] at (1.16,1.08) {Front};
...
\begin{tikzpicture}[x=1in,y=1in]
  \path[use as bounding box] (0,0) rectangle (3.02,1.08);
  ...
  \node[inner sep=0] at (2.66,.54)
    {\includegraphics[width=.55in]{images/rulebook-counters/leven-artillery-1-back.pdf}};
  ...
  \node[piece label] at (1.16,1.04) {Front (ready)};
  \node[piece label] at (2.66,1.04) {Back (captured)};
  \node[piece label,anchor=east] (abtype) at (2.24,.28) {Artillery\\Type Symbol};
  \draw[piece lead] (abtype.north east) -- (2.64,.48);
  \node[piece label,anchor=east] (abcaptured) at (3.00,.82) {Indicates\\Captured};
  \draw[piece lead] (abcaptured.south west) -- (2.66,.63);
\end{tikzpicture}
\endgroup
\end{tcolorbox}
