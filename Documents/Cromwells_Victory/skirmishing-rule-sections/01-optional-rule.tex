...
\begin{enumerate}
  \item The skirmishing unit fires at its declared target, and the target makes
        a \textbf{Reaction Roll}. Roll one die for each unit. The skirmishing
        unit disrupts its target on a roll of \textbf{1 or 2}; the target
        disrupts the skirmishing unit on a Reaction Roll of \textbf{1}.
  \item Record both results, but do not apply either result yet. The two attacks
        ...
        fire.
  \item Apply both recorded results simultaneously, then remove the marker.
\end{enumerate}
...
\begin{rulebox}{Skirmish Fire}
\centering
\begin{tabularx}{\linewidth}{@{}>{\bfseries}l Y@{}}
  \toprule
  Roll & Disrupts on \\
  \midrule
  Skirmishing unit & 1--2 \\
  Reaction & 1 \\
  \bottomrule
\end{tabularx}
\end{rulebox}
...
March Phase movement: \ruleref{8.0}. Disruption: \ruleref{11.8}. Short Game:
\ruleref{16.0}.
\end{rulebox}
